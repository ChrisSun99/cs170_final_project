...
def sanity_check(dist_matrix, clusters, centers, starting_location, homes):
    ...
    for i in range(len(check)):
        if (check[i] > mean + 2.5 * sd and np.isin(i, homes) and len(centers) < 13):
            clusters[i] = i
...
def real_solve(list_of_locations, list_of_homes, starting_car_location, adjacency_matrix):
    ...
    centers, clusters = sanity_check(dist_matrix, clusters, centers, s, homes)
